...
int main(void)
{
    ...
    expected_proof  = (unsigned char *) sodium_malloc(crypto_vrf_ietfdraft10_PROOFBYTES_BATCHCOMPAT);
    ...
    unsigned char proof[crypto_vrf_ietfdraft10_PROOFBYTES_BATCHCOMPAT];
    ...
    assert(crypto_vrf_ietfdraft10_PROOFBYTES_BATCHCOMPAT == 128);
    ...
    for (i = 0U; i < (sizeof test_data) / (sizeof test_data[0]); i++) {
        ...
        sodium_hex2bin(expected_proof, crypto_vrf_ietfdraft10_PROOFBYTES_BATCHCOMPAT,
        ...
        if (crypto_vrf_ietfdraft10_prove_batchcompat(proof, sk, messages[i], i) != 0){
            printf("crypto_vrf_prove() error: [%u]\n", i);
        }
        if (memcmp(expected_proof, proof, crypto_vrf_ietfdraft10_PROOFBYTES_BATCHCOMPAT) != 0){
            printf("proof error: [%u]\n", i);
            printhex("\tWanted: ", expected_proof, crypto_vrf_ietfdraft10_PROOFBYTES_BATCHCOMPAT);
            printhex("\tGot:    ", proof, crypto_vrf_ietfdraft10_PROOFBYTES_BATCHCOMPAT);
        }
        if (crypto_vrf_ietfdraft10_verify_batchcompat(output, expected_pk, proof, messages[i], i) != 0){
        ...
        if (i > 0) {
            if (crypto_vrf_ietfdraft10_verify_batchcompat(output, expected_pk, proof, messages[i], i-1) == 0){
                printf("verify succeeded with truncated message: [%u]\n", i);
            }
        }

        if (crypto_vrf_ietfdraft10_proof_to_hash_batchcompat(output, proof) != 0){
